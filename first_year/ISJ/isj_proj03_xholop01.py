...
def first_task():
    ...
    # inversion using zip
    plur2sing = {value:key for key,value in sing2plur.items()}  # vase reseni
    return plur2sing == {'geese': 'goose', 'men': 'man', 'children': 'child'}


def second_task():
    word2vect = {'king': [3, 1], 'queen': [6, 3], 'uncle': [4, 3], 'aunt': [8, 9]}
    vect2word = {tuple(x):y for y,x in word2vect.items()}  # vase reseni
...
class MyList(list):
    def __hash__(self):
        return hash(str(self))  # nahradte za vase reseni
