...
void const *gpu_computeVertexAttributeDataPointer(
    ...
    assert(head != NULL);
    return (char*) head->buffer + head->offset + gl_VertexID * head->stride;
}
...
void gpu_runVertexPuller(GPUVertexPullerOutput *const              output,
    ...
    const struct GPUVertexPullerHead *head = NULL;
    for (int i=0; i<4; i++) { //  attributes
        head = &(puller->heads[i]);
        if (head->buffer != NULL && head->enabled == 1) {
            output->attributes[i] = gpu_computeVertexAttributeDataPointer(head, v_id);
        } else {
...
void gpu_runPrimitiveAssembly(
    ...
    (void)vertexShader;
    struct GPUVertexPullerOutput out;

    GPUVertexShaderInput sh;
    for (size_t i = 0; i<nofPrimitiveVertices; i++) {
        gpu_runVertexPuller(&out, puller, baseVertexShaderInvocation+i);
        sh.attributes = &out;
        sh.gl_VertexID = gpu_computeGLVertexID(puller->indices, baseVertexShaderInvocation+i);
        vertexShader(&primitive->vertices[i], &sh, gpu);
...
void gpu_computeScreenSpaceBarycentrics(
    ...
    sub_Vec2(&vector0, &vertices[1], &vertices[0]);
    sub_Vec2(&vector1, &vertices[2], &vertices[0]);
    sub_Vec2(&vector2, pixelCenter, &vertices[0]);

    float support[6];

    support[1] = dot_Vec2(&vector0, &vector0);
    support[2] = dot_Vec2(&vector0, &vector1);
    support[3] = dot_Vec2(&vector1, &vector1);
    support[4] = dot_Vec2(&vector2, &vector0);
    support[5] = dot_Vec2(&vector2, &vector1);

    support[0] = support[1]*support[3]-support[2]*support[2];

    coords->data[1] = (support[3]*support[4]-support[2]*support[5])/support[0];
    coords->data[2] = (support[1]*support[5]-support[2]*support[4])/support[0];
