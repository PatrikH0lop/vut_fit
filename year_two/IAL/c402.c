...
**/
	...
	new->LPtr = NULL;
	new->RPtr = NULL;
	//vloz ho ako koren ak je strom prazdny
	if (*RootPtr==NULL) {
		*RootPtr = new;
	}
	else {
		//nerekurzivne prejdeme strom pomocou ukazatela
		tBTNodePtr *node = RootPtr;
		while (*node!=NULL) {
			//hladaj miesto v lavom podstrome
			if ((*node)->Cont > Content) {
				node = &(*node)->LPtr;
			}
			// hladaj miesto v pravom podstrome
			else if ((*node)->Cont < Content) {
				node = &(*node)->RPtr;
			}
			// inak prvok zrus, uz existuje
			else {
				free(new);
				return;
			}
		}
		// vloz prvok na najdene miesto
		*node = new;
	}
}
...
**/
	...
	while (ptr!=NULL) {
		//pushni prvok na spracovanie praveho stromu
		SPushP(Stack, ptr);
		BTWorkOut(ptr);
		//posun do do laveho podstromu
		ptr = ptr->LPtr;
	}
}
...
**/
	...
	tStackP tStack;
	SInitP(&tStack);
	// posun sa v lavom podstrome
	Leftmost_Preorder(RootPtr, &tStack);
	// spracuj prave podstromy prvkov na stacku
	while (!SEmptyP(&tStack)) {
		RootPtr = STopPopP(&tStack);
		Leftmost_Preorder(RootPtr->RPtr, &tStack);
	}
}
...
**/
	...
	while (ptr!=NULL) {
		// pushni prvky pre spracovanie praveho podstromu
		SPushP(Stack, ptr);
		// posun sa do laveho podstromu bez spracovania vrcholu
		ptr = ptr->LPtr;
	}
}
...
**/
	...
	tStackP stack;
	SInitP(&stack);
	// spracuj lavy podstrom
	Leftmost_Inorder(RootPtr, &stack);
	// spracuj vrcholy a nasledne prave podstromy pushnutych vrcholov
	while (!SEmptyP(&stack)) {
...
**/
	...
	while (ptr!=NULL) {
		// pushni vrchol pre neskorsie spracovanie praveho podstromu
		SPushP(StackP, ptr);
		// pushni informaciu o navstiveni
		SPushB(StackB, true);
		// posun sa do laveho podstromu
		ptr = ptr->LPtr;
	}
}
...
**/
	...
	tStackB stackB;
	SInitP(&stackP);
	SInitB(&stackB); // navstivenie vrcholu
	bool left; // indikator ze treba spracovat pravy podstrom, ide zlava

	// posun sa v lavom podstrome
	Leftmost_Postorder(RootPtr, &stackP, &stackB);
	// spracuj pushnute vrcholy
	while (!SEmptyP(&stackP)) {
		RootPtr = STopPopP(&stackP);
		left = STopPopB(&stackB);
		SPushP(&stackP, RootPtr);
		// ak zostal pravy podstrom potrebny na spracovanie vrcholu, spracuj ho
		if (left) {
			SPushB(&stackB, false);
...
**/
	...
	SInitP(&Stack);
	do {
		// nacitaj prvok na spracovanie ak existuje
		if (*RootPtr == NULL) {
			if (!SEmptyP(&Stack)) {
				*RootPtr = STopPopP(&Stack);
			}
			...
			// pridaj pravy podstrom k prvkom na spracovanie
			if ((*RootPtr)->RPtr != NULL) {
				SPushP(&Stack, (*RootPtr)->RPtr);
			}
			//uvolni dany vrchol a posun sa vlavo
