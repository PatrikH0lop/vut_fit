...
INSERT INTO Zamestnanec (id, pozice, pohlavi, cislo_uctu, plat) VALUES (2, 'prodejce', 'muz', '0080/250152', 28000);
...
-- Caje, ktere nejsou z Japonska serazeny podle nazvu
SELECT C.id, C.nazev, Z.nazev_zeme
  FROM Caj C, Zeme Z
  WHERE Z.nazev_zeme=C.zeme_puvodu AND Z.nazev_zeme<>'Japonsko'
  ORDER BY Z.nazev_zeme;

-- Caje od stejneho dodavatele
SELECT C1.id AS Caj_1_id, C2.id AS Caj_2_id, C1.nazev AS Caj_1, C2.nazev AS Caj_2, C1.dodavatel
  FROM Caj C1, Caj C2
  WHERE C1.dodavatel=C2.dodavatel AND C1.id<C2.id;

-- Kontinenty, odkud caje pochazeji
SELECT DISTINCT kontinent
  FROM Caj LEFT JOIN Zeme ON zeme_puvodu=nazev_zeme;

-- Vsechny varky, ktere jsou drazsi nez 65 korun/100g, a jsou z Japonska nebo Maroka (id varky, cena, zeme_puvodu)
SELECT id AS id_varky, V.cena_100g, Z.nazev_zeme
  FROM Varka V Natural join Caj C left join Zeme Z ON C.zeme_puvodu=Z.nazev_zeme
  WHERE (Z.nazev_zeme='Japonsko' OR Z.nazev_zeme='Maroko') AND V.cena_100g>65;

-- Jake mnozstvi mame ve varkach z ruznych zemi
SELECT Z.nazev_zeme, SUM(V.mnozstvi) AS Celkove_mnozstvi
  FROM Varka V, Caj C, Zeme Z
  WHERE V.id_caj=C.id AND C.zeme_puvodu=Z.nazev_zeme
  GROUP BY nazev_zeme;

-- Podobne jako predchozi priklad, ale vypise pouze zeme, ze kterych mame mene nez 10 kg
SELECT Z.nazev_zeme, SUM(V.mnozstvi) AS Celkove_mnozstvi
  FROM Varka V, Caj C, Zeme Z
  WHERE V.id_caj=C.id AND C.zeme_puvodu=Z.nazev_zeme
  GROUP BY nazev_zeme
  HAVING SUM(V.mnozstvi)<10;

-- Objednavky, ktere vyrizoval zamestnanec 'Fero Malachosky' (Vypisuje jejich ID pro pripad ze mame 2 Fery)
-- Vypise id objednavky, sledovaci cislo a stav
SELECT O.id, V.sledovaci_cislo, O.stav
  FROM Vyrizuje V, Objednavka O
  WHERE V.id_objednavka=O.id AND V.id_zamestnanec IN
    (SELECT Z.id
      FROM Zamestnanec Z LEFT JOIN Osoba Os ON Z.id=Os.id
      WHERE Os.jmeno='Fero Malachosky');
